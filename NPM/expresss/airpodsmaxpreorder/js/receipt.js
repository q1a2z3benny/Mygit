...
function RaidoCheckedInvoice(i) {
    $('input[name="radInvoice"]')[1].checked = true;
    $('input[name="radInvoice"]')[i].checked = true;
    $(".e_invoice div").hide();
    $("#panInvoice" + i).show();
    if (i == 1) {
        RaidoCheckedInvoice2($('input:radio[name=radivno]:checked').val() - 1);
    } else if (i == 0) {
        RaidoCheckedInvoice3($('input:radio[name=radivno2]:checked').val());
    }
}

function RaidoCheckedInvoice2(i) {
    $('input[name="radivno"]')[i].checked = true;
    $("#iv_no0").hide();
    $("#iv_no1").hide();
    $("#iv_no" + i).show();
}
...
function checkCart() {
    ...
    msg += checkName('name', '付款人姓名');
    msg += checkItem('mobile', '付款人手機');
    if (checkItem('mobile', '付款人手機').length == 0) {
        msg += checkPhone('mobile', '付款人手機');
    }
    msg += checkItem('UCAddrZipCode1_ddlCity', '付款人連絡地址');
    if (checkItem('UCAddrZipCode1_ddlCity', '付款人連絡地址').length == 0) {
        msg += checkItem('UCAddrZipCode1_ddlTown', '付款人連絡地址');
    }
    if (checkItem('UCAddrZipCode1_ddlTown', '付款人連絡地址').length == 0) {
        msg += checkItem('UCAddrZipCode1_txtAddr', '付款人連絡地址');
    }
